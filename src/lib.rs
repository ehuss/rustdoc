...
impl Config {
    ...
    pub fn new(verbosity: Verbosity, manifest_path: PathBuf) -> Result<Config> {
        ...
        if !manifest_path.is_file() || !manifest_path.ends_with("Cargo.toml") {
            return Err(failure::error_msg("The --manifest-path must be a path to a Cargo.toml file").into());
        }
...
pub fn build(config: &Config, artifacts: &[&str]) -> Result<()> {
    ...
    if artifacts.contains(&"frontend") {
        ...
        if !output.status.success() {
            ...
            return Err(format_err!("frontend `{}` did not execute successfully", frontend).into());
        }
    }
...
pub fn test(config: &Config) -> Result<()> {
    let doc_json = File::open(config.documentation_path()).map_err(|_|
        failure::error_msg("could not find generated documentation")
    )?;
