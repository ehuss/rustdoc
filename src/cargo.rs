    ...
    F: Fn(&str) -> (),
{
    ...
    let target_dir = config
        .manifest_path
        .parent()
        .ok_or(failure::error_msg("Expected manifest_path to point to Cargo.toml"))?
        .join("target/rls");
    ...
    if !status.success() {
        Err(error::Cargo{ status, stderr }.into())
    } else {
        Ok(())
    }
}
...
pub fn target_from_metadata(ui: &Ui, metadata: &serde_json::Value) -> Result<Target> {
    ...
    if targets.is_empty() {
        return Err(failure::error_msg("no targets with supported kinds (`bin`, `lib`) found").into());
